...
createApp({
    ...
    setup() {
        ...
        // --- 生图服务配置（可在设置面板中自定义） ---
        // 留空时回退到原项目作者的服务；填写自定义地址后不再请求该域名。
        const DEFAULT_IMAGE_GEN_BASE_URL = 'https://nai.sta1n.cn';
        // 默认路径模板（GET 直接取图）。可用占位符：{prompt} {token} {artist} {size} {negative}
        ...
        // 渲染出的占位 <img> 由 MutationObserver 接管，异步补上真实图片地址
        ...
        const fetchQuota = async () => {
            ...
            if (!imageGenToken) {
                ...
                // 配额接口是原服务专有的，使用自定义生图地址时跳过，避免无意义的跨域请求
                if (!isDefaultImageGenService()) {
                    quotaValue.value = 0;
                    return;
                }
...
#### 更新时间：08/03
        ...
        const updateImageGenRegexState = ({ enableRegex = false } = {}) => {
            ...
            if (isOpenAIImageGen()) {
                // OpenAI 兼容模式的 replacement 是内嵌 SVG 占位图的 <img>，不是查询串。
                // 对它做 URL 参数替换会命中占位图里的 font-size= 并截断整段 HTML，
                // 连 data-imagegen-prompt 一起吞掉，导致观察器永远扫不到目标、一次请求都不发。
                // 画师与尺寸在 POST 时由 getImageGenArtists() / getOpenAIImageSize() 现取，无需写进 replacement。
                if (openAIImageGenAppliedArtists !== targetArtists) {
                    openAIImageGenAppliedArtists = targetArtists;
                    messages.push(styleName);
                }
                if (openAIImageGenAppliedSize !== settings.imageSize) {
                    openAIImageGenAppliedSize = settings.imageSize;
                    messages.push(`比例: ${settings.imageSize}`);
                }
            } else {
                // 动态替换 URL 中的 artist 和 size 参数。
                // 只在 src="..." 内部改写，防止把标签上的其它属性/内嵌样式当成查询串误伤。
                const encodedTargetArtists = encodeURIComponent(targetArtists);
                const oldReplacement = regex.replacement;
                const srcPattern = /(<img\b[^>]*?\bsrc=")([^"]*)(")/;
                const oldSrc = oldReplacement.match(srcPattern)?.[2] || '';
                let newSrc = oldSrc.replace(/artist=[\s\S]*?(&size=)/, 'artist=' + encodedTargetArtists + '$1');
                if (newSrc === oldSrc) {
                    newSrc = oldSrc.replace(/artist=[^&]+/, 'artist=' + encodedTargetArtists);
                }
                newSrc = newSrc.replace(/size=[^&]+/, 'size=' + settings.imageSize);
                if (newSrc !== oldSrc) {
                    regex.replacement = oldReplacement.replace(srcPattern, (match, prefix, url, suffix) => prefix + newSrc + suffix);
                }

                // 检查 Artist 变化
                const oldArtist = oldSrc.match(/artist=([\s\S]*?)&size=/)?.[1] || oldSrc.match(/artist=([^&]+)/)?.[1];
                if (oldArtist !== encodedTargetArtists) {
                    messages.push(styleName);
                }
                // 检查 Size 变化
                const oldSize = oldSrc.match(/size=([^&]+)/)?.[1];
        ...
        const buildExecutableHtmlDocument = (rawHtml) => {
            ...
            const jqueryScript = '<script src="' + (new URL('assets/vendor/jquery.min.js', location.href)).href + '" defer><\/script>';
            const scriptShim = `
                <script>
...
${content}
        ...
        const renderMarkdown = (text, role = 'assistant', skipRegex = false) => {
            ...
            const cleanConfig = {
                ...
                ADD_ATTR: ['style', 'open', 'srcdoc', 'sandbox', 'frameborder', 'allow', 'allowfullscreen', 'class', 'id', 'viewBox', 'fill', 'stroke', 'stroke-width', 'd', 'stroke-linecap', 'stroke-linejoin', 'x1', 'y1', 'x2', 'y2', 'offset', 'stop-color', 'stop-opacity', 'width', 'height', 'onclick', 'type', 'value', 'checked', 'data-slash', 'data-imagegen-prompt', 'data-imagegen-state'],
        ...
        const checkImageGenStatus = async () => {
            ...
            // 想确认"真的能出图"请用下方的「测试生图连接」，那个会真实拉一张图。
            await checkConnectionStatus(imageGenStatus, imageGenLatency, 'Image API', signal => (
                fetch(getImageGenBaseUrl(), {
                    method: 'HEAD',
                    mode: 'no-cors',
                    signal
                })
        ...
        // --- 连接测试：真实发一次请求，把成功/失败原因直接显示出来 ---
        // 状态灯只说明"能连上"，这里的测试说明"真的能出结果"。
        const CONNECTION_TEST_PROMPT = '1girl, simple background, upper body';
        // OpenAI 系模型有内容策略，NAI 风格的测试词会被判违规（content_policy_violation），
        // 所以 OpenAI 兼容模式单独用一条中性提示词。
        ...
        // 把底层报错翻译成能看懂的原因。手机上没有控制台，信息必须落到界面上。
        ...
        // 光看"能不能解码成图片"不够：默认服务在密钥无效时会返回
        // HTTP 200 + image/svg+xml 的"生成失败"占位图，能正常解码，会被误判成功。
        // 所以先取响应体，按 content-type 判断，并把 SVG 里的失败文案提取出来当报错。
        ...
        // 只有真的解码出图片才算成功，返回 JSON 错误体或 HTML 报错页时 naturalWidth 为 0。
        ...
        const testImageGenConnection = async () => {
            ...
            try {
                ...
                let weak = false;
                if (isOpenAIImageGen()) {
                    // 不带画风词，避免画师名触发 OpenAI 的内容策略，测试只验证通路
        ...
        const testApiConnection = async () => {
            ...
            try {
                ...
                // 流式单独探一次：App 内的原生 HTTP 转发不支持 SSE，
                // 探到不支持就直接告诉用户去关掉流式，而不是让对话莫名失败。
                let streamNote = '';
                if (settings.stream) {
                    try {
