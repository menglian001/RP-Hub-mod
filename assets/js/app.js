...
const app = createApp({
    ...
    setup() {
        ...
        // --- Image Generation Multi-Mode Support (from mod) ---
        const DEFAULT_IMAGE_GEN_BASE_URL = IMAGE_GEN_BASE_URL;
        ...
        const isAgnesImageGenMode = () => settings.imageGenMode === IMAGE_GEN_MODE_AGNES;
        const isOpenAIImageGenMode = () => settings.imageGenMode === IMAGE_GEN_MODE_OPENAI;
        const isVertexImageGenMode = () => settings.imageGenMode === IMAGE_GEN_MODE_VERTEX;
        const isComfyImageGenMode = () => settings.imageGenMode === IMAGE_GEN_MODE_COMFY;
        ...
        const getImageGenProfile = () => {
            ...
            if (isComfyImageGenMode()) {
                return {
                    key: String(settings.comfyImageGenKey || '').trim(),
                    url: String(settings.comfyImageGenUrl || '').trim(),
                    model: String(settings.comfyImageGenModel || '').trim()
                };
            }
            return {
        ...
        const buildImageGenUrl = ({ prompt = '$1', token = '', artist = '', size = '' } = {}) => {
            const placeholders = {
                '{prompt}': prompt,
                '{token}': encodeURIComponent(token),
                '{artist}': artist,
                '{size}': encodeURIComponent(size),
                '{negative}': DEFAULT_IMAGE_GEN_NEGATIVE
            };
            const path = getImageGenPath().replace(
                /\{(prompt|token|artist|size|negative)\}/g,
                (match) => (placeholders[match] !== undefined ? placeholders[match] : '')
            );
        ...
        // --- End Image Generation Multi-Mode ---
        ...
        const vertexImageSizeOptions = [
            ...
            { value: 'vertex_1_1', label: '方图 1:1' },
            { value: 'vertex_9_16', label: '竖长图 9:16' },
            { value: 'vertex_16_9', label: '宽图 16:9' }
        ];
        // --- End Image Library State ---
        ...
        const normalizeActiveTool = (tool = {}) => {
            ...
            // 世界书工具的历史别名统一收敛到 tool_world（二改功能，上游 1.8.x 曾移除）
            const isWorldTool = legacyWorldToolNames.includes(rawCallName)
                || ['world_info', 'world_info_list', 'world_info_read', 'world_info_edit'].includes(tool.type)
                || legacyWorldToolNames.includes(tool.id);
            ...
            const callName = isWorldTool ? 'tool_world' : (isLegacyWebTool ? 'tool_web' : rawCallName);
            const defaultToolId = isWorldTool ? 'tool_world' : (isLegacyWebTool ? 'tool_web' : tool.id);
            const defaultTool = getDefaultActiveToolDefinitions()
                .find(item => item.id === defaultToolId || item.callName === callName);
            ...
            if (normalizedType === ACTIVE_TOOL_WORLD_TYPE) {
                const accessModeVersion = Number(tool.worldInfoAccessModeVersion) || 1;
                const rawMode = tool.worldInfoAccessMode || tool.worldInfoMode || tool.accessMode || fallback.worldInfoAccessMode;
                normalized.worldInfoAccessMode = normalizeWorldInfoAccessMode(rawMode);
                normalized.worldInfoAccessModeVersion = ACTIVE_TOOL_WORLD_ACCESS_VERSION;
                // 动态更新 description 和 displayDescription 以反映当前权限模式
        ...
        const addRoleImage = (roleUuid, image) => {
            ...
            const saveTask = previousSave.then(async () => {
                ...
                if (imageLibraryRoleUuid.value === roleKey || currentCharacter.value?.uuid === roleKey) {
                    roleImages.value = images;
                }
                return record;
            });
            roleImageSaveQueues.set(roleKey, saveTask);
            saveTask.catch(() => { }).finally(() => {
        ...
        const deleteRoleImages = async (roleUuid) => {
            if (!roleUuid) return;
            await deleteScopedStoredValue('images', roleUuid).catch(() => { });
            if (currentCharacter.value?.uuid === roleUuid) roleImages.value = [];
            if (imageLibraryRoleUuid.value === String(roleUuid)) {
                imageLibraryRoleUuid.value = '';
                roleImages.value = [];
            }
        };
        // === End Image Library Storage ===
        ...
        // === Mod Features: Image Library View ===
        const imageLibraryCharacter = computed(() => (
            characters.value.find(char => String(char?.uuid || '') === imageLibraryRoleUuid.value) || null
        ));
        ...
        const filteredRoleImages = computed(() => {
            ...
            return roleImages.value.filter(image => (
                (imageLibraryCategory.value === 'all'
                    || (imageLibraryCategory.value === 'favorite'
                        ? image.favorite === true
                        : image.category === imageLibraryCategory.value))
        ...
        const toggleRoleImageFavorite = async (image) => {
            const roleUuid = imageLibraryRoleUuid.value || currentCharacter.value?.uuid;
            if (!roleUuid || !image?.id) return;
            const images = roleImages.value.map(item => (
                item.id === image.id ? { ...item, favorite: !item.favorite } : item
            ));
            await saveRoleImages(roleUuid, images);
            roleImages.value = images;
        };
        ...
        watch(() => currentCharacter.value?.uuid, (uuid) => {
            if (currentView.value !== 'images') return;
            imageLibraryRoleUuid.value = String(uuid || '');
            loadCurrentRoleImages();
        });
        // === End Image Library View ===
        ...
        const checkImageGenStatus = async () => {
            ...
            if (isOpenAIImageGenMode()) {
                ...
                await checkConnectionStatus(imageGenStatus, imageGenLatency, 'Image API', signal => {
                    const base = getImageGenApiEndpoint().replace(/\/images\/generations$/, '');
                    const url = /\/v\d+$/.test(base) ? `${base}/models` : `${base}/v1/models`;
                    const headers = {};
                    const key = getActiveImageGenKey();
                    if (key) headers['Authorization'] = `Bearer ${key}`;
                    return fetch(url, { headers, signal });
                });
                return;
            }
            // GET 直链模式实际取图走 <img src>，不受跨域限制，这里沿用宽松探测
            await checkConnectionStatus(imageGenStatus, imageGenLatency, 'Image API', signal => (
        ...
        const getActiveImageGenKey = () => {
            ...
            if (isComfyImageGenMode()) return settings.comfyImageGenKey;
        ...
        const postOpenAIImage = async (fullPrompt, responseFormat, signal, style = '', options = {}) => {
            ...
            if (!response.ok) {
                const detail = await response.text().catch(() => '');
                const error = new Error(`HTTP ${response.status} ${detail.slice(0, 160)}`.trim());
        ...
        const requestComfyImage = async (prompt, options = {}) => {
            ...
            return postOpenAIImage(fullPrompt, 'b64_json', options.signal, '', {
                image_count: Math.min(8, Math.max(1, Number(settings.imageGenCount) || 1)),
        ...
        const fetchImageGenModels = async (isManual = false) => {
            ...
            if (!isPostImageGenMode()) {
                if (isManual) showToast('GET 直链模式无需选择模型', 'info');
                return;
            }
            if (!profile.url && !isAgnesImageGenMode() && !isComfyImageGenMode()) {
                if (isManual) showToast('请先填写生图接口地址', 'info');
                return;
            }
        ...
        // ============================================================
        // === Mod Features: POST Image Gen Pipeline ===
        // ============================================================
        // GET 直链模式走上游的 job 卡片；POST（OpenAI/Agnes/Vertex/Comfy）模式
        // 无法用 <img src> 直接取图，先渲染占位图，再由 MutationObserver 回填。
        const imageGenMemoryCache = new Map();
        const imageGenInflight = new Map();
        // 生图不在 isGenerating 生命周期内，单独登记在飞控制器，让中止按钮也能停掉
        ...
        const imageGenCacheKey = (prompt) => `imagegen_${hashForImageGen([
            ...
            isComfyImageGenMode() ? settings.comfyImageGenModel : '',
        ...
        const startImageGenRequest = (img, prompt, rememberImage) => {
            const startedAt = Date.now();
        ...
        const hydrateImageGenPlaceholder = (img) => {
            if (!img || !img.dataset) return;
            const state = img.dataset.imagegenState;
            ...
            // 切页面后 DOM 重建时，先查 IndexedDB 是否已有生成结果
            getStoredValue(cacheKey).then(stored => {
                ...
                startImageGenRequest(img, prompt, rememberImage);
            }).catch(() => {
                startImageGenRequest(img, prompt, rememberImage);
            });
        };
        ...
        const startImageGenObserver = () => {
            ...
            scanImageGenPlaceholders(document.body);
        };

        const stopImageGenObserver = () => {
            imageGenObserver?.disconnect();
        ...
        const replaceRoleImageSource = async (roleUuid, oldSrc, prompt, newSrc) => {
            ...
            if (index >= 0) {
                ...
                await saveRoleImages(roleUuid, images);
                if (imageLibraryRoleUuid.value === String(roleUuid) || currentCharacter.value?.uuid === roleUuid) {
                    roleImages.value = images;
                }
                return;
            }
        ...
        // 向量记忆与关键词检索两类工具的判定（二改保留；上游 1.8.5+ 已移除向量工具）
        ...
        // ============================================================
        // === Mod Features: World Info Tool ===
        // ============================================================
        const ACTIVE_TOOL_WORLD_TYPE = activeToolConfig.types.world;
        const ACTIVE_TOOL_WORLD_ACCESS_VERSION = activeToolConfig.worldInfoAccess.version;
        const ACTIVE_TOOL_WORLD_ACCESS_READ = activeToolConfig.worldInfoAccess.read;
        const ACTIVE_TOOL_WORLD_ACCESS_EDIT = activeToolConfig.worldInfoAccess.edit;

        const normalizeWorldInfoAccessMode = (raw) => {
            const mode = String(raw || '').trim().toLowerCase();
            if (['edit', 'write', '编辑', '写入'].includes(mode)) return ACTIVE_TOOL_WORLD_ACCESS_EDIT;
            return ACTIVE_TOOL_WORLD_ACCESS_READ;
        };

        const getWorldInfoToolDescription = (accessMode) => (
            activeToolConfig.worldInfoAccess.descriptions[normalizeWorldInfoAccessMode(accessMode)]
        );

        const getWorldInfoToolDisplayDescription = (accessMode) => (
            activeToolConfig.worldInfoAccess.displayDescriptions[normalizeWorldInfoAccessMode(accessMode)]
        );
        ...
        const getEnabledWorldInfoToolEntries = () => {
            return worldInfo.value
                .map((entry, index) => ({ entry: normalizeWorldInfoEntry(entry), index, sourceIndex: index }))
                .filter(item => item.entry.enabled !== false && !systemWorldInfoNames.includes(item.entry.comment));
        };

        const normalizeWorldInfoToolQuery = (query) => String(query || '').trim()
            .replace(/^(?:list|列表|all|全部|read|阅读|view|查看|edit|编辑|update|修改)\s*[:：]?\s*/i, '');

        const parseWorldInfoJsonPayload = (query) => {
            const text = normalizeWorldInfoToolQuery(query);
        ...
        const getWorldInfoTargetFromPayload = (payload, fallback = '') => {
            if (!payload) return String(fallback || '').trim();
            return String(payload.name || payload.comment || payload.target || payload.id || fallback || '').trim();
        };

        const isWorldInfoAllQuery = (query) => {
            const text = normalizeWorldInfoToolQuery(query);
            return !text || ['list', '列表', 'all', '全部'].includes(text.toLowerCase());
        };

        const resolveWorldInfoToolEntries = (target, { includeContentMatch = false, limit = ACTIVE_TOOL_MAX_RESULT_COUNT } = {}) => {
            const entries = getEnabledWorldInfoToolEntries();
            if (!target) return entries.slice(0, limit);
            const idMatch = String(target).match(/^#?(\d+)$/);
            if (idMatch) {
                const index = Number(idMatch[1]);
                return entries.filter(item => item.index === index).slice(0, 1);
            }
            const lowerTarget = String(target).toLowerCase();
            const exactMatches = entries.filter(item => String(item.entry.comment || '').toLowerCase() === lowerTarget);
            if (exactMatches.length > 0) return exactMatches.slice(0, limit);
            const partialMatches = entries.filter(item => {
                const comment = String(item.entry.comment || '').toLowerCase();
                const keys = (item.entry.keys || []).map(key => String(key || '').toLowerCase());
                if (comment.includes(lowerTarget)) return true;
                if (keys.some(key => key.includes(lowerTarget))) return true;
                if (includeContentMatch) {
                    return String(item.entry.content || '').toLowerCase().includes(lowerTarget);
                }
                return false;
            });
        ...
        const formatWorldInfoEntryForTool = (item, { includeContent = false } = {}) => {
            const entry = item.entry;
            const content = String(entry.content || '').trim();
            return {
                index: item.index,
                sourceIndex: item.sourceIndex,
                comment: entry.comment || `世界书 ${item.index}`,
        ...
        const listEnabledWorldInfoForTool = () => {
            ...
            results.totalEnabledCount = matches.length;
            ...
            return BUILTIN_PROMPTS.buildActiveToolSystemPrompt({
                tools: tools.map(tool => {
                    ...
                    return {
                        ...
                        kind: isWorldInfoActiveTool(tool)
                            ? 'world'
                            : (isWebActiveTool(tool) ? 'web' : (isKeywordActiveTool(tool) ? 'keyword' : 'vector')),
                        worldCanEdit: isWorldInfoActiveTool(tool) ? canEditWorldInfoWithTool(tool) : false
                    };
                }),
        ...
        const generateResponse = async (startTime = null, options = {}) => {
            ...
            try {
                ...
                if (assistantMessage?.content) {
                    // 二改行为：文风过滤只收集命中、不改写正文，避免删掉用户想看的内容。
                    // 但仍要把命中项挂到消息上，1.8.8 的「文风过滤详情」弹层依赖它。
                    const styleFilterHits = [];
                    filterBlockedStyleText(assistantMessage.content, {
                        log: true,
        ...
        const formatActiveToolResultContext = (tool, query, results, mode = 'add') => {
            ...
            if (isWorldInfoActiveTool(tool)) {
                ...
                const formattedEntries = results.map(item => {
                    ...
                    const contentText = indentXmlText(item.content || '', 4);
                    const previewText = !item.content && item.preview ? indentXmlText(item.preview, 4) : '';
                    return [
                ...
                return [
                    `<active_tool_result name="${title}" call="${callName}" mode="${modeValue}" query="${escapeXmlAttribute(cleanQuery)}" world_info_mode="read">`,
                    `  <description>以下是系统根据查询从已开启世界书中读取到的条目内容。${modeDescription}本段内容由系统插入最后一条用户消息结尾。请优先依据这些世界书内容继续回答；如果需要编辑世界书，必须在用户明确要求时才执行。</description>`,
                    formattedEntries,
                    '</active_tool_result>'
                ].join('\n');
            }
        ...
        const deleteCharacterData = async (char, legacyIndex, knownStorageKeys = null) => {
            ...
            if (imageLibraryRoleUuid.value === String(char.uuid)) {
                imageLibraryRoleUuid.value = '';
                imageLibraryPreview.value = null;
                roleImages.value = [];
            }
        ...
        const enforceSpecialRules = () => {
            ...
            const imageGenBaseUrl = getImageGenBaseUrl();

            // 1. NAI画图正则（根据当前模式动态生成）
            const imageGenRegexName = 'NAI画图正则';
            const targetArtists = cardUtils.getImageStyleArtists(settings.imageStyle, settings.customImageArtists);
            const encodedTargetArtists = encodeURIComponent(targetArtists);
            const imageGenSrc = buildImageGenUrl({
                prompt: '$1',
                token: imageGenToken,
                artist: encodedTargetArtists,
            ...
            const imageGenRegexContent = {
                ...
                regex: '/image###([^\\r\\n]*?)(?:###|(?=\\r?\\n)|$)/g',
                replacement: imageGenReplacement,
        ...
        // === Mod Features: Announcement System ===
        // 启动时检查线上版本，有新公告则弹窗
